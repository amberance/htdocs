   ...
   <div class="i_modal_in_in i_sf_box"> 
       <div class="i_modal_content">  
          ...
          <div class="purchase_post_details tabing"> 
             ...
             <script>
             var details = <?php echo json_encode($DataUserDetails) ?>;
             //alert(details['paymentOption']);
             //alert(details.order_id);
             var bitcoinurl = "https://www.amberance.com/boxcoin/pay.php?checkout_id=custom-"+details.order_id + "&price="+details.amounts.USD+"&external_reference=" + details.customer_id;
             //alert(bitcoinurl);
             //document.getElementById("bitcoinurl").href = bitcoinurl;
              ...
              <a id="bitcoin-link" href="https://www.amberance.com/boxcoin/pay.php?checkout_id=<?php echo json_encode('order_id') ?>&price=<?php echo json_encode('price') ?>&external_reference=111">
              
              <img src="/img/bitcoin-wide.jpg" width="100%"/>
              <?php 
              
              
              $jsonobj = json_encode($DataUserDetails);
              $obj = json_decode(@jsonobj, true);
              
              
              //echo $jsonobj[0][1];
              //echo $jsonobj->amounts;
              //echo $jsonobj->customer_id; 
              
              //echo $obj->paymentOption;
              //echo $obj[0][1];
              //echo $jsonobj[customer_id]; 
              ...
              <div class="payment_method_box transition payMethod" id="bitcoin" data-type="bitcoin">
                  <div class="payment_method_item flex_ bitcoin">CLICK BITCOIN!!!</div>
              </div>
              
              <div class="payment_method_box transition paywithCrip" id="bitcoin" data-type="bitcoin">
                  <div class="payment_method_item flex_ coinpayment">xxx</div>
              </div>
...
$(document).ready(function() {
    ...
    $("body").on("click", ".payMethod", function(e) {
        ...
        console.log("The paywidth is: " + payWidth);
        var plan = $(".mp").attr("data-p");
        var planID = '<?php echo filter_var($productID, FILTER_SANITIZE_STRING);?>';
        var type = 'processProduct'; 
        ...
        if(payWidth == 'bitcoin' || payWidth == 'paypal' || payWidth == 'iyzico' || payWidth == 'authorize-net' || payWidth == 'bitpay'){
            $.ajax({
                type: 'post', //form method
                context: this,
                url: siteurl + 'my-payment-process.php', // post data url
                dataType: "JSON",
                ...
                console.log("DB Update Success!!!");
                    ...
                    console.log("About to forward after db update");
                    if(payWidth == 'paypal'){  
                        $(".lw-show-till-loading").show();
                        //on success load paypalUrl page 
                        window.location.href = response.paypalUrl;
                    }else if (payWidth == 'bitcoin'){
        console.log("going to bitcoin address!");
                    	//alert("bitocoin!!! chaning url");
...
//         if(payWidth == 'bitcoin') {
//         	console.log("Moving to payment pae for bitcoin");
//			window.location.replace("/text-pay2.php" + '?f='+type+'&paymentOption=' + payWidth + '&creditPlan=' + '&' + $.param(JSON.parse('<?php echo json_encode($DataUserDetails) ?>'))); //done
//         }
        
        
    });
    $("body").on("click",".paywithCrip", function(){
    console.log("Pying with cryp");
        var planID = '<?php echo filter_var($productID, FILTER_SANITIZE_STRING);?>';
        var payWidth = $(this).attr("data-type");
        var plan = $(".mp").attr("data-p");
        var type = 'cop';
        var data = 'f='+type+'&p='+planID;
        $.ajax({
            type: 'POST',
            ...
            success: function(response) {
             console.log("Success");
                ...
                if(redirect && status == '200'){
                    ...
                    alert('Something went wrong in your CoinPayment Settings');
                }
            }
        });
    });
