    ...
    ***********/
    ...
    function showMoreData() {
        if (scrollLoad && $(window).scrollTop() >= $(document).height() - $(window).height() - 500) {
            ...
            if (moreType == 'profile' && moreCat != '') { var ID = $('#moreType').children('.i_post_body').last().attr('data-last'); var profileUserID = $("#prw").attr("data-u"); }
            if ($('.i_loading , .nomore , .noPost , .no_creator_f_wrap').length === 0 && !$(".i_loading , .nomore , .noPost , .no_creator_f_wrap")[0] && moreType != undefined) {
                if (moreCat != '' || moreCat != undefined) {
                    var data = 'f=' + moreType + '&last=' + ID + '&p=' + profileUserID + '&pcat=' + moreCat;
                } else {
