        ...
        print: function (onSuccess, search = transactions_filters[0], status = transactions_filters[1], cryptocurrency = transactions_filters[2], date_range = transactions_filters[3]) {
            this.get((response) => {
                ...
                for (var i = 0; i < pagination_count; i++) {
                    ...
                    code += `<tr data-id="${transaction.id}" data-cryptocurrency="${transaction.cryptocurrency}" data-hash="${transaction.hash}" data-status="${transaction.status}"${transaction.billing ? ' data-invoice="true"' : ''}><td class="bcx-td-id">${transaction.id}</td><td class="bcx-td-time"><div class="bxc-title">${BOXCoin.beautifyTime(transaction.creation_time, true)}</div></td><td class="bxc-td-title"><div class="bxc-title">${transaction.title}</div><div class="bxc-text">${transaction.description}</div></td><td>${from}</td><td>${transaction.to ? `<a href="${BXCAdmin.explorer(transaction.cryptocurrency, transaction.to)}" target="_blank" class="bxc-link">${transaction.to}</a>` : ''}</td><td class="bxc-td-status"><span class="bxc-status-${transaction.status}">${bxc_(transaction_statuses[transaction.status])}</span></td><td class="bxc-td-amount"><div class="bxc-title">${transaction.amount ? transaction.amount + ' ' + BOXCoin.baseCode(transaction.cryptocurrency.toUpperCase()) : amount}</div><div class="bxc-text">${transaction.amount ? amount : BOXCoin.baseCode(transaction.cryptocurrency)}</div></td><td><i class="bxc-transaction-menu-btn bxc-icon-menu"></i></td></tr>`;
                }
    ...
    */
    ...
    var BXCAdmin = {
            ...
            if (!loading(area)) {
                ...
                ajax('get-balances', {}, (response) => {
                    ...
                    for (var key in balances) {
                        code += `<tr data-cryptocurrency="${key}"><td><div class="bxc-flex"><img src="${BXC_URL}media/icon-${key}.svg" /> ${balances[key].name}${BOXCoin.network(key)}</div></td><td><div class="bxc-balance bxc-title">${balances[key].amount} ${BOXCoin.baseCode(key).toUpperCase()}</div><div class="bxc-text">${BXC_CURRENCY} ${balances[key].fiat}</div></td></tr>`;
                    }
        ...
        explorer: function (cryptocurrency, value, type = 'address') {
            let explorer = '';
            let replace = type == 'address' ? 'address' : (cryptocurrency == 'ltc' ? 'transaction' : 'tx');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        */
        ...
        transactions_table.on('click', '.bxc-transaction-menu-btn:not(.bxc-loading)', function () {
            ...
            let code = '<ul class="bxc-transaction-menu bxc-ul">';
            ...
            if (!active) {
                activate(this);
                if (row.attr('data-invoice') && status == 'C') code += `<li data-value="invoice">${bxc_('Invoice')}</li>`;
                if (row.find('.bxc-td-title .bxc-text').html().length > 30) code += `<li data-value="description">${bxc_('Description')}</li>`;
        ...
        transactions_table.on('click', '.bxc-transaction-menu li', function () {
            ...
            if (['C', 'P', 'R'].includes(value)) {
                ajax('update-transaction', { transaction_id: id, values: { status: value } }, () => {
                    row.attr('data-status', value).find('.bxc-td-status span').attr('class', 'bxc-status-' + value).html(bxc_(transaction_statuses[value]));
                    loading(menu, false);
                });
            }
            if (value == 'invoice') {
                ajax('invoice', { transaction_id: id }, (response) => {
                    window.open(response);
                    loading(menu, false);
                });
            }
            if (value == 'description') {
                row.find('.bxc-td-title .bxc-text').addClass('bxc-description-full');
                loading(menu, false);
            }
        ...
        */
        ...
        window.onscroll = function () {
            if ((document.documentElement || document.body.parentNode || document.body).scrollTop + window.innerHeight == _.documentHeight() && pagination_count) {
                if (active_area_id == 'transactions') {
