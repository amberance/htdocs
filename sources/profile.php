...
if($p_MessageCanSend == '1'){
   $sendMessage = '<div class="i_btn_item transition"><div class="newMessageMe flex_ tabing ownTooltip" data-label="'.filter_var($LANG['send_message'], FILTER_SANITIZE_STRING).'" id="'.$p_profileID.'">'.$iN->iN_SelectedMenuIcon('38').'</div></div>';
   if($logedIn == 1){
     $checkChatStartedBefore = $iN->iN_GetConverationID($userID, $p_profileID);
     if($checkChatStartedBefore){
      $sendMessage = '<div class="i_btn_item transition tabing ownTooltip" data-label="'.filter_var($LANG['send_message'], FILTER_SANITIZE_STRING).'"><a "href="'.$base_url.'chat?chat_width='.$checkChatStartedBefore.'">'.$iN->iN_SelectedMenuIcon('38').'</a></div>';
     }
   }
}
