...
class iN_UPDATES {
	...
	*/
	...
	public function iN_InsertUserSubscription($userID, $subscribedUserID, $planType, $subscriberName, $subscrID, $custID, $planIDs, $planAmount, $adminEarning, $userNetEarning, $planCurrency, $planinterval, $planIntervalCount, $subscriberEmail, $plancreated, $current_period_start, $current_period_end, $planStatus) {
		...
		if ($this->iN_CheckUserExist($userID) == 1 && $this->iN_CheckUserExist($subscribedUserID) == 1) {
			...
			if ($query) {
				...
				} else if($this->iN_CheckUserIsInSubscriber($userID, $subscribedUserID) != 1){
	...
	public function iN_UpdateSubscriptionStatus($subscriptionID) {
		...
		mysqli_query($this->db, "UPDATE i_user_subscriptions SET status = 'inactive', in_status = '1', finished = '1' WHERE subscription_id = '$subscriptionID'") or die(mysqli_error($this->db));
	}
	public function iN_AllUserProfilePosts($uid, $lastPostID, $showingPost) {
		...
		$data = null;
		$getData = mysqli_query($this->db, "SELECT P.*,U.*
		FROM i_friends F 
			INNER JOIN i_posts P 
			ON P.post_owner_id = F.fr_two
			INNER JOIN i_users U 
			ON P.post_owner_id = U.iuid AND U.uStatus IN('1','3') AND F.fr_status IN('me', 'flwr', 'subscriber')
	...
	public function iN_ChatUserList($userID, $limit) {
		...
		if ($this->iN_CheckUserExist($userID) == '1') {
			$query = mysqli_query($this->db, "SELECT DISTINCT
		  C.chat_id,C.user_one,C.user_two,U.iuid,U.i_username,U.i_user_fullname,U.user_avatar,U.online_offline_status,U.last_login_time,U.user_gender,U.user_verified_status
	...
	public function iN_ExploreUserLatestFivePost($userID) {
		...
		FROM i_posts P FORCE INDEX (ixForcePostOwner)
			INNER JOIN i_users U FORCE INDEX (ixForceUser)
			ON P.post_owner_id = U.iuid
		WHERE P.post_owner_id='$userID' AND U.uStatus IN('1','3') AND P.post_file <> '' AND P.post_status = '1' AND P.post_file IS NOT NULL
		ORDER BY P.post_id
	...
	public function iN_UpdateVerificationProfileStatus($userID, $answerType, $answerValue, $answeringVerificationID) {
		...
		if ($this->iN_CheckIsAdmin($userID) == '1' && $this->iN_CheckVerificationRequestExist($answeringVerificationID) == 1) {
			...
			if ($answerType == '1') {
				...
				mysqli_query($this->db, "UPDATE i_users SET certification_status = '0', validation_status = '0',condition_status = '0', fees_status = '0' , payout_status = '0', payout_method = NULL , paypal_email  = NULL , bank_account = NULL WHERE iuid = '$iuIDfk'") or die(mysqli_error($this->db));
				return true;
			}
		} else {
			return false;
	...
	public function iN_YourPaymentsList($userID, $paginationLimit, $page) {
		...
		if ($this->iN_CheckUserExist($userID) == 1) {
			...
			WHERE P.payment_status = 'ok' AND P.payer_iuid_fk = '$userID' AND P.payment_type IN('post','live_stream','tips','live_gift','videoCall') ORDER BY P.payment_id DESC LIMIT $start_from, $paginationLimit
			") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query)) {
				$data[] = $row;
			}
	...
	public function iN_EPDUpdate($userID, $maxPoint, $epdRegisterStatus, $epdCommentStatus, $epdNewPostStatus, $epdCommetLikeStatus, $epdPostLikeStatus, $epdRegisterAmount, $epdCommendAmount, $epdCommentLikeAmount, $epdNewPostAmount, $epdPostLikeAmount) {
		...
		$query11 = mysqli_query($this->db, "UPDATE i_configurations SET max_point_in_a_day = '$maxPoint' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
	...
	public function iN_FriendStoryPost($userID) {
		...
		if ($this->iN_CheckUserExist($userID) == 1) { 
			$query = mysqli_query($this->db, "
				...
				AND F.fr_one = '$userID'
				AND (F.fr_status = 'fri' OR F.fr_status = 'me' OR F.fr_status = '1' OR F.fr_status = 'flwr' OR F.fr_status = 'subscriber')
				GROUP BY S.uid_fk
	...
	public function iN_FriendStoryPostListAll($userID) {
		...
		if ($this->iN_CheckUserExist($userID) == 1) { 
			$query = mysqli_query($this->db, "
				...
				AND (F.fr_status = 'fri' OR F.fr_status = 'me' OR F.fr_status = '1' OR F.fr_status = 'flwr' OR F.fr_status = 'subscriber')
				GROUP BY S.uid_fk
				ORDER BY S.uid_fk ASC LIMIT 100") or die(mysqli_error($this->db));
					while ($row = mysqli_fetch_array($query)) {
						$data[] = $row;
					}
					if (!empty($data)) {
	...
	public function iN_TotalProductSell($pID) {
		$pID = mysqli_real_escape_string($this->db, $pID);
		$countProductSeen = mysqli_query($this->db, "SELECT COUNT(*) AS productSellCount FROM i_user_payments WHERE paymet_product_id = '$pID' AND payment_status = 'ok' AND payed_iuid_fk IS NOT NULL") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($countProductSeen, MYSQLI_ASSOC);
...
//            $query = mysqli_query($this->db,"SELECT * FROM i_user_payments WHERE paymet_product_id = '$productID' AND payer_iuid_fk = '$userID'  AND payed_iuid_fk IS NOT NULL") or die(mysqli_error($this->db));
		    $query = mysqli_query($this->db,"SELECT * FROM i_user_payments WHERE paymet_product_id = '$productID' AND payer_iuid_fk = '$userID'  AND payed_iuid_fk IS NOT NULL AND payment_status = 'ok'") or die(mysqli_error($this->db));
		...
		if ($this->iN_CheckUserExist($userID) == 1) {
			$query = mysqli_query($this->db, "SELECT DISTINCT P.*,U.*,X.*
				FROM i_users U FORCE INDEX(ixForceUser)
					...
					INNER JOIN i_user_product_posts X FORCE INDEX (ixProduct)
					ON X.pr_id = P.paymet_product_id AND U.uStatus IN('1','3') AND P.payment_type IN('product')
				WHERE P.payer_iuid_fk='$userID' AND X.pr_id = P.paymet_product_id AND P.payment_status = 'ok' AND P.payed_iuid_fk IS NOT NULL
				ORDER BY P.payed_iuid_fk
				DESC LIMIT $start_from, $paginationLimit ") or die(mysqli_error($this->db)); 
			while ($row = mysqli_fetch_array($query)) {
				$data[] = $row;
	...
	public function iN_GetVideoCallDetailsBetweenTwoUsersIfExistIM($chatID, $userID){
		$query = mysqli_query($this->db,"SELECT * FROM i_video_call WHERE chat_id_fk = '$chatID' AND (called_uid_fk = '$userID' OR caller_uid_fk = '$userID') AND accept_status = '2'") or die(mysqli_error($this->db));
		if(mysqli_num_rows($query) > 0){
             return true;
		}else{
			return false;
	 ...
	 public function iN_GetVideoCallDetailsBetweenTwoUsersIfExist($chatID, $userID){
		$query = mysqli_query($this->db,"SELECT * FROM i_video_call WHERE chat_id_fk = '$chatID' AND called_uid_fk = '$userID' AND accept_status = '2'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		/*Return result if icon id exist or not*/
		return $data;
	}
	...
	public function iN_InsertVideoCall($userID, $videoCallName, $calledUserID) {
		if($this->iN_CheckUserExist($userID) == 1 && $this->iN_CheckUserExist($calledUserID) == 1){
            ...
            if($this->iN_CheckConversationStartedBeforeBetweenUsers($userID, $calledUserID) == 0){
				$queryChat = mysqli_query($this->db, "INSERT INTO i_chat_users (user_one, user_two)VALUES('$userID','$calledUserID')") or die(mysqli_error($this->db));
				if($queryChat){
					$getChatID = mysqli_query($this->db, "SELECT * FROM i_chat_users WHERE user_one = '$userID' AND user_two = '$calledUserID'") or die(mysqli_error($this->db));
			        $row = mysqli_fetch_array($getChatID, MYSQLI_ASSOC);
					if (mysqli_num_rows($getChatID) == 1) {
						$time = time();
						$chatID = isset($row['chat_id']) ? $row['chat_id'] : NULL;
						$query = mysqli_query($this->db, "INSERT INTO i_video_call (chat_id_fk,voice_call_name, called_uid_fk, caller_uid_fk, called_time)VALUES('$chatID','$videoCallName','$calledUserID','$userID','$time')") or die(mysqli_error($this->db));
						return $chatID;
					}
				}  
			}else{ 
				$getChatID = mysqli_query($this->db, "SELECT * FROM i_chat_users WHERE (user_one = '$userID' AND user_two = '$calledUserID') OR (user_one = '$calledUserID' AND user_two = '$userID')") or die(mysqli_error($this->db));
				$row = mysqli_fetch_array($getChatID, MYSQLI_ASSOC);
				if (mysqli_num_rows($getChatID) == 1) {
					$time = time();
					$chatID = isset($row['chat_id']) ? $row['chat_id'] : NULL;
	...
	public function iN_CheckVideoCall($userID) {
		if($this->iN_CheckUserExist($userID) == 1){
			$query = mysqli_query($this->db,"SELECT * FROM i_video_call WHERE called_uid_fk = '$userID' AND accept_status = '1' AND DATE_FORMAT(now(), '%H-%i') = DATE_FORMAT(FROM_UNIXTIME(called_time), '%H-%i')") or die(mysqli_error($this->db));
			$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
			/*Return result if icon id exist or not*/
			return isset($data['vc_id']) ? $data['vc_id'] : FALSE;
		}
	}
	public function iN_CheckVideoCallStatus($userID) {
		if($this->iN_CheckUserExist($userID) == 1){
			$query = mysqli_query($this->db,"SELECT * FROM i_video_call WHERE caller_uid_fk = '$userID' AND DATE_FORMAT(now(), '%H-%i') = DATE_FORMAT(FROM_UNIXTIME(called_time), '%H-%i')") or die(mysqli_error($this->db));
			$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
			/*Return result if icon id exist or not*/
			return isset($data['accept_status']) ? $data['accept_status'] : FALSE;
		}
	}
	public function iN_VideoCallDetails($vID){ 
		$vDetails = mysqli_query($this->db, "SELECT * FROM `i_video_call` WHERE vc_id = '$vID'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($vDetails, MYSQLI_ASSOC);
		return $data;
	}
	public function iN_VideoCallAcceptDetails($vID){ 
		mysqli_query($this->db, "UPDATE i_video_call SET accept_status = '2' WHERE vc_id = '$vID'") or die(mysqli_error($this->db));
		$vDetails = mysqli_query($this->db, "SELECT * FROM `i_video_call` WHERE vc_id = '$vID'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($vDetails, MYSQLI_ASSOC);
		return $data;
	}
	public function iN_CheckAndDeleteCall($userID, $channelName) {
		$query = mysqli_query($this->db,"SELECT * FROM i_video_call WHERE voice_call_name = '$channelName'") or die(mysqli_error($this->db));
		if(mysqli_num_rows($query) == '1'){
			mysqli_query($this->db, "DELETE FROM i_video_call WHERE voice_call_name = '$channelName'");
			return true;
		}else{
			return false;
		}
	...
	public function iN_VideoCallDeclineDetails($vID){ 
		mysqli_query($this->db, "UPDATE i_video_call SET accept_status = '3' WHERE vc_id = '$vID'") or die(mysqli_error($this->db));
		$vDetails = mysqli_query($this->db, "SELECT * FROM `i_video_call` WHERE vc_id = '$vID'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($vDetails, MYSQLI_ASSOC);
		return $data;
	}
	...
	public function iN_UpdateUsersWalletsForVideoCall($userID, $tiSendingUserID, $tipAmount, $netUserEarning,$adminFee,$adminEarning,$userNetEarning){
		...
		if ($this->iN_CheckUserExist($userID) == 1 && $this->iN_CheckUserExist($tiSendingUserID) == 1) { 
			...
			mysqli_query($this->db, "UPDATE i_users SET wallet_money = wallet_money + '$userNetEarning' WHERE iuid = '$tiSendingUserID'") or die(mysqli_error($this->db));$time = time();
            mysqli_query($this->db, "INSERT INTO i_user_payments(payer_iuid_fk, payed_iuid_fk,payment_type,payment_time,payment_status, amount, user_earning,admin_earning,fee)
			VALUES('$userID','$tiSendingUserID', 'videoCall', '$time', 'ok', '$netUserEarning', '$userNetEarning','$adminEarning','$adminFee')") or die(mysqli_error($this->db));
			return true;
		}
	} 
	/*Update Post Create Mod*/
	public function iN_UpdateVideoCallFeatureStatus($userID, $mod) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE  i_configurations SET video_call_feature_status = '$mod' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateWhoCanCreateVideoCallFeatureStatus($userID, $mod) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_configurations SET who_can_careate_video_call = '$mod' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateIsVideoCallPaidStatus($userID, $mod) {
		if ($this->iN_CheckIsAdmin($userID) == 1) {
			mysqli_query($this->db, "UPDATE i_configurations SET is_video_call_free = '$mod' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function iN_UpdateVideoCost($userID, $videoCost) {
		if($this->iN_CheckUserExist($userID) == 1){
            mysqli_query($this->db,"UPDATE i_users SET video_call_price = '$videoCost' WHERE iuid = '$userID'") or die(mysqli_error($this->db));
			return true;
		}else{
			return false;
		}
	...
	public function iN_TotalEarningPointsInaDay($userID) {
		$userID = mysqli_real_escape_string($this->db, $userID); //  DATE(FROM_UNIXTIME(pointed_time))=CURDATE()
		if($this->iN_CheckUserExist($userID) == 1){
			$CountLike = mysqli_query($this->db, "SELECT SUM(point) AS earningPoints FROM i_user_point_earnings WHERE DATE(FROM_UNIXTIME(pointed_time))=CURDATE() AND poninted_user_id = '$userID'") or die(mysqli_error($this->db));
	...
	public function iN_MovePointEarningsToPointBalance($userID, $totalPointEarned){
		if($this->iN_CheckUserExist($userID) == 1){
			$queryOne = mysqli_query($this->db, "UPDATE i_user_point_earnings SET calculated_point = '1' WHERE poninted_user_id = '$userID'") or die(mysqli_error($this->db));
			$queryTwo = mysqli_query($this->db, "UPDATE i_users SET wallet_points = wallet_points + '$totalPointEarned' WHERE iuid = '$userID'") or die(mysqli_error($this->db));
